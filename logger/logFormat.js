...
const formatProduction = format.combine(timeFormat, format.printf((info) => {
    return `${info.timestamp} - [${info.level.toUpperCase().padEnd(2)}] - ${info.message}`
}));


const formatDevelopment = format.combine(timeFormat, format.printf((info) => {
    return colorize.colorize(info.level, `${info.timestamp} - [${info.level.toUpperCase().padEnd(2)}] - ${info.message}`)
}));


module.exports = {
    formatProduction, formatDevelopment
}